...
> September 12, 2026: the probability tasks below are historical. Continue from [audit implementation status](AUDIT_IMPLEMENTATION.md), including the [rank-test calculators](NONPARAMETRIC_VALIDATION.md), [repeated-measures ANOVA](REPEATED_MEASURES_VALIDATION.md), [shared t-test/ANOVA workflow](ANALYSIS_WORKFLOW_VALIDATION.md), and [correlation/regression workflow](TABULAR_WORKFLOW_VALIDATION.md). The [first complete Learning Lab path](LEARNING_LAB_VALIDATION.md) is implemented. The [descriptive source review and simple-regression saved launches](DESCRIPTIVE_WORKFLOW_VALIDATION.md) are also implemented. The [saved-row review and descriptive input recovery](ROW_REVIEW_DRAFT_VALIDATION.md) are implemented. The [simple regression calculator draft](SIMPLE_REGRESSION_DRAFT_VALIDATION.md) is implemented, including source, roles, confidence, prediction, and plot settings. The [multiple regression calculator draft](MULTIPLE_REGRESSION_DRAFT_VALIDATION.md) is implemented, including separate role and prediction profiles for each data source. The [Pearson calculator draft](PEARSON_DRAFT_VALIDATION.md) is implemented, including source-specific variable choices, hypothesis direction, confidence, null correlation, and plot settings. The [one-sample t-test calculator draft](ONE_SAMPLE_DRAFT_VALIDATION.md) is implemented, with separate raw/summary inputs, explicit saved-data copies, validated inference, and a copyable report. The [independent-samples t-test draft](INDEPENDENT_DRAFT_VALIDATION.md) is implemented, with pooled/Welch inference, separate group inputs, persistent group order, saved-data copies, and reports. The [paired t-test calculator draft](PAIRED_DRAFT_VALIDATION.md) is implemented, with intact matched rows, separate raw/summary profiles, saved-data copies, and Student t intervals. The [paired lesson inference/report parity](PAIRED_LESSON_VALIDATION.md) is implemented, with shared Student t inference, reports, explicit example-pair plots, and validated lesson inputs. The [one-way ANOVA calculator draft](ONE_WAY_DRAFT_VALIDATION.md) is implemented, with separate raw/summary/F profiles, current saved-data copies, validated equal-variance inference, Bonferroni comparisons, and reports. The [one-way ANOVA lesson parity](ANOVA_LESSON_VALIDATION.md) is implemented, including shared calculator inference/reports, honest observed-versus-summary plots, keyboard plot controls, variance/table explanations, and the active app report builder. Next: expand draft recovery to factorial ANOVA, then ANCOVA and repeated measures. Continue consistent report exports and manual-input exclusion review. Descriptive draft recovery currently saves only input text and source labels, not display settings. Descriptive inputs remain single-variable lists; saved-data launches for these modules are a separate extension. Repeated-measures power and mixed/factorial repeated designs remain unsupported; do not substitute independent-groups models.
