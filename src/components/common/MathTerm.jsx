...
const MathTerm = ({ term, onInfo, onHover, darkMode, value, showValue }) => {
  ...
  const displayValue =
    showValue && value !== undefined ? valDisplay : cleanTerm;

  const tooltip = isCalculable
    ? `${MATH_TERMS[actualTerm].desc}${showValue && value !== undefined ? ` (Value: ${valDisplay})` : ""}`
    : "";
  ...
  const sharedProps = {
    ...
    title: tooltip,
    className: `inline-block px-1 mx-0.5 rounded transition-all sym-link ${
      isCalculable
        ? darkMode
          ? "cursor-pointer hover:bg-indigo-500/20 text-indigo-400 font-bold"
          : "cursor-pointer hover:bg-indigo-100 text-indigo-900 font-bold"
        : darkMode
          ? showValue
