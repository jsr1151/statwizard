...
const TUTOR_PRESETS = [
    ['strong_positive', 'Strong Positive', 'Clear upward linear trend.'],
    ['weak_positive', 'Weak Positive', 'Upward trend with much more noise.'],
    ['near_zero', 'Near Zero', 'Little straight-line association.'],
    ['strong_negative', 'Strong Negative', 'Clear downward linear trend.'],
    ['nonlinear', 'Nonlinear', 'Curved pattern with weak Pearson r.'],
    ['outlier', 'Outlier Example', 'One point can move r a lot.'],
    ['restricted_range', 'Restricted Range', 'Limited spread can shrink r.'],
];
...
const PearsonCorrelationPage = ({
    ...
    const [lessonOutlierOn, setLessonOutlierOn] = useState(true);

    const lessonPairs = useMemo(() => generatePearsonTutorDataset({
        preset: lessonPreset,
                    ...
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className={`mt-2 text-sm max-w-3xl ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                This stays on the shared StatWizard power surface. It plans designs from alpha, tails, sample size, rho0, and the expected correlation under H1 instead of mixing in observed-data calculator output.
                            </p>
                        </div>
                    </div>
                </Card>
                        ...
                        <div className="flex items-start justify-between gap-4">
                            <div>
                                ...
                                <h3 className={`text-2xl font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                                    Pearson r = {formatStat(effectSourceStats.r, 3)}
                                </h3>
                                <p className={`mt-2 text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                        ...
                        <Card darkMode={darkMode} className="h-full">
                            <div className="flex items-center gap-3 mb-4">
                                ...
                                <div>
                                    ...
                                    <h3 className={`text-2xl font-black ${darkMode ? 'text-white' : 'text-slate-900'}`}>
                                        Pearson r is already the effect size
                                    </h3>
                                </div>
                            </div>

                            <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                This section stays simple on purpose: translate between r and r^2, keep context visible, and avoid pretending that one set of conventions fits every study.
                            </p>
                    ...
                    <div className="lg:col-span-7 space-y-6">
                        ...
                        <Card darkMode={darkMode}>
                            ...
                            <p className={`text-sm leading-relaxed ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                r^2 reframes the correlation as shared linear variance. Many users find it easier to read &quot;about 25% of the variance is shared&quot; than to interpret r = .50 directly.
                            </p>
                        </Card>
                ...
                <Card darkMode={darkMode}>
                    <div className="flex items-start gap-4">
                        ...
                        <div>
                            ...
                            <p className={`mt-2 text-sm max-w-3xl ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                                Upload or paste a table, choose X and Y, then inspect Pearson r, r^2, the test result, the confidence interval, and the scatterplot-based warnings.
                            </p>
                        </div>
                    </div>
                </Card>
                        ...
                        <Card darkMode={darkMode}>
                            ...
                            {!parsedTable.ok ? (
                                ...
                                <div className="space-y-4">
                                    ...
                                    <label className="block">
                                        <span className={`text-[11px] font-black uppercase tracking-widest ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}>Null Correlation (rho0)</span>
                                        <input
                                            type="number"
                                            min={-0.95}
                                            max={0.95}
                        ...
                        {!calculatorStats?.ok ? (
                            ...
                            <>
                                <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
                                    <MetricTile darkMode={darkMode} label="Pearson r" value={formatStat(calculatorStats.r, 3)} tone="primary" />
                                    <MetricTile darkMode={darkMode} label="r^2" value={formatStat(calculatorStats.rSquared, 3)} />
                                    <MetricTile darkMode={darkMode} label="Sample Size (n)" value={`${calculatorStats.n}`} />
                                    <MetricTile darkMode={darkMode} label="Interpretation" value={calculatorStats.interpretation} />
                                </div>

                                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-4">
                                    <MetricTile darkMode={darkMode} label={`${calculatorStats.hypothesisTest?.statisticLabel || 't'} Statistic`} value={formatStat(calculatorStats.hypothesisTest?.testStatistic, 3)} detail={calculatorStats.hypothesisTest?.method === 'fisher_z' ? 'Fisher-z approximation for nonzero rho0.' : 'Exact t test when rho0 = 0.'} />
                                    ...
                                    <MetricTile darkMode={darkMode} label="p-value" value={formatPValue(calculatorStats.hypothesisTest?.pValue)} detail={`Tested against rho0 = ${formatStat(rho0, 2)}.`} />
                                </div>
            ...
            <Card darkMode={darkMode}>
                <div className="flex items-start gap-4">
                    ...
                    <div>
                        ...
                        <p className={`mt-2 text-sm max-w-3xl ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                            This first slice is an interactive concept page rather than a formal lesson engine. Use the presets and controls to see what Pearson r responds to and what it misses.
                        </p>
                    </div>
                </div>
            </Card>
            ...
            <div className="grid lg:grid-cols-12 gap-8 items-start">
                <div className="lg:col-span-8 space-y-6">
                    <Card darkMode={darkMode}>
                        <PearsonScatterplot
                            ...
                            subtitle={TUTOR_PRESETS.find((preset) => preset[0] === lessonPreset)?.[2]}
                        />
                    </Card>

                    <div className="grid sm:grid-cols-2 xl:grid-cols-4 gap-4">
                        <MetricTile darkMode={darkMode} label="Pearson r" value={formatStat(lessonStats?.r, 3)} tone="primary" />
                        <MetricTile darkMode={darkMode} label="r^2" value={formatStat(lessonStats?.rSquared, 3)} />
                        <MetricTile darkMode={darkMode} label="Sample Size (n)" value={`${lessonStats?.n || 0}`} />
                ...
                <div className="lg:col-span-4 space-y-6">
                    <Card darkMode={darkMode}>
                        ...
                        <div className="mt-6 grid grid-cols-2 gap-3">
                            ...
                            <button
                                ...
                                disabled={lessonPreset !== 'outlier'}
                                className={`col-span-2 rounded-xl border px-4 py-3 text-xs font-black uppercase tracking-widest transition-all ${lessonPreset !== 'outlier' ? (darkMode ? 'bg-slate-950/50 border-slate-800 text-slate-600 cursor-not-allowed' : 'bg-slate-50 border-slate-200 text-slate-400 cursor-not-allowed') : lessonOutlierOn ? 'bg-indigo-600 text-white border-indigo-500' : (darkMode ? 'bg-slate-950 border-slate-800 text-slate-400 hover:text-slate-200' : 'bg-slate-50 border-slate-200 text-slate-600 hover:text-slate-900')}`}
                            >
                                {lessonPreset === 'outlier' ? (lessonOutlierOn ? 'Turn Outlier Off' : 'Turn Outlier On') : 'Outlier Toggle Lives in Outlier Preset'}
                            </button>
                        </div>
                    </Card>
                    ...
                    <Card darkMode={darkMode}>
                        ...
                        <div className="space-y-3">
                            {[
                                'Pearson r measures direction and strength of a straight-line association.',
                                'r = 0 does not prove there is no relationship of any kind.',
                                'A curved relationship can look strong in the plot while Pearson r stays small.',
                                'Outliers can strongly change both the fitted line and r.',
                                'Restriction of range usually shrinks Pearson r.',
