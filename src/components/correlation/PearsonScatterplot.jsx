...
const PearsonScatterplot = ({
    ...
    const geometry = useMemo(() => {
        ...
        const bandPoints = showConfidenceBand && stats?.ok
            ? buildRegressionBand({
                stats,
                confidenceLevel,
            })
        ...
        const regressionPoints = stats?.ok
            ...
            : [];
        const confidencePolygon = bandPoints.length > 1
            ? [
                    ...
                    {showLine && (
                        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                            ...
                            <p className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                The fitted line summarizes the average straight-line trend in the points.
                            </p>
                        </div>
                    )}
