...
export default function CentralTendencyPage({ section, darkMode }) {
    ...
    ].map(([name, use, caution]) => <Card key={name} darkMode={darkMode}><h4 className="text-lg font-black text-indigo-400">{name}</h4><p className="mt-3 text-sm"><strong>Useful when:</strong> {use}</p><p className={`mt-3 text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}><strong>Caution:</strong> {caution}</p></Card>)}</div></div>;
    ...
    return <div className="space-y-6"><Card darkMode={darkMode}><div className="flex items-start gap-4"><div className="p-3 rounded-xl bg-indigo-500/10 text-indigo-400"><BookOpen /></div><div><h3 className="text-xl font-black">What does “typical” mean?</h3><p className={`mt-2 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>Central tendency describes a distribution with a representative value. Mean, median, and mode answer different versions of that question.</p></div></div></Card><div className="grid gap-4 md:grid-cols-3">{[['Mean','The balance point','Uses every observation and changes when any value changes.'],['Median','The positional center','Half the ordered observations are at or below it and half are at or above it.'],['Mode','The most frequent','The only one of the three generally appropriate for nominal categories.']].map(([name,subtitle,text]) => <Card key={name} darkMode={darkMode}><Lightbulb className="text-amber-400" /><h4 className="mt-4 text-lg font-black">{name}</h4><p className="text-sm font-bold text-indigo-400">{subtitle}</p><p className={`mt-3 text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{text}</p></Card>)}</div><Card darkMode={darkMode}><h3 className="font-black">Remember</h3><ul className={`mt-3 space-y-2 text-sm list-disc pl-5 ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}><li>Report sample size and a measure of variability alongside center.</li><li>A large outlier pulls the mean toward it but usually has much less effect on the median.</li><li>A dataset can be multimodal or have no uniquely most frequent value.</li><li>Do not average numeric category codes whose distances have no quantitative meaning.</li></ul></Card></div>;
}
