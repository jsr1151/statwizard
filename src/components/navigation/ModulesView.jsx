      ...
      <div className="max-w-4xl mx-auto animate-in fade-in slide-in-from-right-8 duration-500">
        <div className="flex items-center gap-4 mb-10">
          <button
            ...
            aria-label="Back to statistical module families"
        ...
        <div className="grid md:grid-cols-2 gap-4">
          {familyStats.map((p) => (
            ...
            >
              <span
                className={`text-base font-bold transition-colors ${darkMode ? "text-slate-300 group-hover:text-white" : "text-slate-700 group-hover:text-indigo-700"}`}
              >
                {p.title}
              </span>
              <div
                className={`p-2 rounded-xl border transition-all duration-300 ${darkMode ? "bg-slate-950 border-slate-800 group-hover:border-indigo-500" : "bg-slate-50 border-slate-200 group-hover:border-indigo-500"}`}
              >
                <Play
                  aria-hidden="true"
                  className={`w-4 h-4 translate-x-0.5 ${darkMode ? "text-slate-700 group-hover:text-indigo-400" : "text-slate-400 group-hover:text-indigo-600"}`}
                />
              </div>
            </button>
          ))}
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-500 text-left">
      <div className="text-center max-w-2xl mx-auto mb-8">
        <h2
          className={`text-4xl font-black mb-3 ${darkMode ? "text-white" : "text-slate-900"}`}
        >
          Statistical Modules
        </h2>
        <p
        ...
        >
          Select a family or tool directly to start your analysis.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-8">
        {categories.map((cat) => {
          ...
          const familyList = [
            ...new Set(
              STAT_PAGE_LIST.filter((p) => p.category === cat && p.family).map(
                (p) => p.family,
              ),
            ),
          ];
          // Standalone modules
          const standaloneList = STAT_PAGE_LIST.filter(
            (p) => p.category === cat && !p.family,
          );

          return (
            <div
              key={cat}
              className={`p-6 rounded-3xl border border-dashed ${darkMode ? "bg-slate-900/40 border-slate-800" : "bg-slate-50/50 border-slate-200"}`}
            >
              <h3
                className={`text-xs font-black uppercase tracking-widest mb-6 px-2 ${darkMode ? "text-indigo-400" : "text-indigo-600"}`}
              >
                {cat}
              </h3>
              <div className="space-y-3">
                {familyList.map((fam) => (
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div
                        className={`p-2.5 rounded-xl ${darkMode ? "bg-slate-950 text-indigo-400" : "bg-indigo-50 text-indigo-600"}`}
                      >
                        {FAMILIES[fam] ? (
                          React.createElement(FAMILIES[fam].icon, {
                            "aria-hidden": true,
                            className: "w-5 h-5",
                          })
                        ) : (
                      ...
                      <div className="text-left">
                        ...
                        <p
                          className={`text-[10px] font-bold uppercase tracking-wider ${darkMode ? "text-slate-600" : "text-slate-400"}`}
                        >
                          {
                            STAT_PAGE_LIST.filter((p) => p.family === fam)
                              .length
                          }{" "}
                          Modules
                        </p>
                    ...
                    <ArrowRight
                      aria-hidden="true"
                      className={`w-4 h-4 transition-transform group-hover:translate-x-1 ${darkMode ? "text-slate-700 group-hover:text-indigo-400" : "text-slate-300 group-hover:text-indigo-600"}`}
                    />
                  </button>
                ))}
                ...
                {standaloneList.map((p) => (
                  ...
                  >
                    <span
                      className={`text-sm font-bold transition-colors ${darkMode ? "text-slate-300 group-hover:text-white" : "text-slate-700 group-hover:text-indigo-700"}`}
                    >
                      {p.title}
                    </span>
                    <div
                    ...
                    >
                      <Play
                        aria-hidden="true"
                        className={`w-3 h-3 translate-x-0.5 ${darkMode ? "text-slate-700 group-hover:text-indigo-400" : "text-slate-400 group-hover:text-indigo-600"}`}
                      />
                    </div>
                  </button>
                ))}
