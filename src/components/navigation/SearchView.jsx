  ...
  return (
    <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div className="text-center mb-12">
        ...
        >
          Search across all modules and concepts.
        </p>
      </div>
      ...
      <div className="relative mb-12">
        <Search
          aria-hidden="true"
          className={`absolute left-6 top-1/2 -translate-y-1/2 w-6 h-6 ${darkMode ? "text-slate-600" : "text-slate-400"}`}
        />
        <label htmlFor="stat-search" className="sr-only">
      ...
      <div className="space-y-4">
        {filtered.length > 0 ? (
          filtered.map((p) => (
            ...
            >
              <div
                className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-colors ${darkMode ? "bg-slate-950 text-slate-700 group-hover:bg-indigo-600 group-hover:text-white" : "bg-slate-50 text-slate-400 group-hover:bg-indigo-600 group-hover:text-white"}`}
              >
                <Activity aria-hidden="true" className="w-5 h-5" />
              </div>
              <div className="text-left">
                <h4
                  className={`text-lg font-black transition-colors ${darkMode ? "text-white group-hover:text-indigo-300" : "text-slate-800 group-hover:text-indigo-700"}`}
                >
                  {p.title}
                </h4>
                <span
                  className={`text-xs font-black uppercase tracking-widest ${darkMode ? "text-slate-600" : "text-slate-400"}`}
                >
                  {p.category}
                </span>
              </div>
          ...
          <div className="py-20 text-center">
            <div
              className={`inline-flex w-16 h-16 rounded-full items-center justify-center mb-4 ${darkMode ? "bg-slate-800 text-slate-600" : "bg-slate-100 text-slate-400"}`}
            >
              <AlertCircle aria-hidden="true" className="w-8 h-8" />
            </div>
            <p
              className={`font-bold ${darkMode ? "text-slate-600" : "text-slate-400"}`}
            >
