...
const PowerAnalysisHub = ({ darkMode, onOpenCalculator }) => {
  ...
  const selectedTest = useMemo(
    () =>
      testsForFamily.find((test) => test.id === selectedTestId) ||
      testsForFamily[0],
  ...
  return (
    <div className="space-y-10 animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="grid lg:grid-cols-12 gap-6 items-start">
        <div
          className={`rounded-3xl border p-6 lg:col-span-3 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
        >
          <div
            className={`text-xs font-black uppercase tracking-widest mb-4 ${darkMode ? "text-indigo-400" : "text-indigo-600"}`}
          >
            1. Test Family
          </div>
          <div className="space-y-3">
            {families.map((familyId) => (
              ...
              >
                ...
                <div
                  className={`text-sm mt-1 ${darkMode ? "text-slate-500" : "text-slate-600"}`}
                >
        ...
        <div
          className={`rounded-3xl border p-6 lg:col-span-5 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
        >
          <div
            className={`text-xs font-black uppercase tracking-widest mb-4 ${darkMode ? "text-indigo-400" : "text-indigo-600"}`}
          >
        ...
        <div
          className={`rounded-3xl border p-6 lg:col-span-4 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
        >
          <div
            className={`text-xs font-black uppercase tracking-widest mb-4 ${darkMode ? "text-indigo-400" : "text-indigo-600"}`}
          >
      ...
      <div
        className={`rounded-3xl border p-6 flex flex-col md:flex-row md:items-center md:justify-between gap-6 ${darkMode ? "bg-slate-900 border-slate-800" : "bg-white border-slate-200"}`}
      >
        <div>
          <div
