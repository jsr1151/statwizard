...
const buildLinePath = (points) => points.map((point, index) => `${index === 0 ? 'M' : 'L'} ${point.x.toFixed(2)} ${point.y.toFixed(2)}`).join(' ');
  ...
  return (
    <div className="space-y-4">
      <div className={`rounded-2xl border ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
        <svg
          role="img"
          aria-label={`${curveModel.yLabel} by ${curveModel.xLabel}. ${curveModel.currentPointSummary}`}
          viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`}
          className="w-full h-auto"
        >
          {chartGeometry.yTicks.map((tick) => (
            <g key={`y-${tick.value}`}>
              <line x1={MARGIN.left} x2={CHART_WIDTH - MARGIN.right} y1={tick.y} y2={tick.y} stroke={gridColor} strokeWidth="1" />
          ...
          {chartGeometry.xTicks.map((tick, index) => (
            <g key={`x-${index}`}>
              <line x1={tick.x} x2={tick.x} y1={MARGIN.top} y2={CHART_HEIGHT - MARGIN.bottom} stroke={gridColor} strokeWidth="1" />
              <text x={tick.x} y={CHART_HEIGHT - MARGIN.bottom + 20} textAnchor="middle" fill={labelColor} fontSize="11" fontWeight="700">
                {formatTick(tick.value, curveModel.curveType)}
              </text>
            </g>
          ))}

          <line x1={MARGIN.left} x2={CHART_WIDTH - MARGIN.right} y1={CHART_HEIGHT - MARGIN.bottom} y2={CHART_HEIGHT - MARGIN.bottom} stroke={axisColor} strokeWidth="1.5" />
          <line x1={MARGIN.left} x2={MARGIN.left} y1={MARGIN.top} y2={CHART_HEIGHT - MARGIN.bottom} stroke={axisColor} strokeWidth="1.5" />

          <path d={buildLinePath(chartGeometry.plottedPoints)} fill="none" stroke={lineColor} strokeWidth="3" strokeLinecap="round" strokeLinejoin="round" />
          ...
          <line
            ...
            opacity="0.45"
          />
          <circle cx={chartGeometry.currentPoint.x} cy={chartGeometry.currentPoint.y} r="6" fill={markerColor} stroke={darkMode ? '#0f172a' : '#ffffff'} strokeWidth="2.5" />

          <text x={(CHART_WIDTH - MARGIN.right + MARGIN.left) / 2} y={CHART_HEIGHT - 10} textAnchor="middle" fill={textColor} fontSize="12" fontWeight="800">
            {curveModel.xLabel}
          </text>
      ...
      <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_minmax(0,1.2fr)]">
        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
          <div className={`text-[10px] font-black uppercase tracking-widest mb-2 ${darkMode ? 'text-indigo-300' : 'text-indigo-700'}`}>Current Design</div>
          <p className={`text-sm ${darkMode ? 'text-slate-200' : 'text-slate-700'}`}>{curveModel.currentPointSummary}</p>
        </div>
        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
          <div className={`text-[10px] font-black uppercase tracking-widest mb-2 ${darkMode ? 'text-slate-500' : 'text-slate-500'}`}>Held Constant</div>
          <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>{curveModel.assumptions}</p>
        </div>
      </div>
    </div>
  );
