...
export default function GamblersFallacyPanel({ darkMode, gamblerStreak, setGamblerStreak, stats, setStats }) {
  ...
  return (
    <div className="animate-in fade-in space-y-6">
      ...
      <div className={`p-5 rounded-3xl border ${darkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200 shadow-sm'} space-y-5`}>
        <div className="text-center">
          ...
          <div className="inline-flex gap-2 mt-3">
            {['H', 'T'].map((side) => <button key={side} type="button" aria-pressed={prediction === side} onClick={() => setPrediction(side)} className={`px-6 py-2 rounded-xl font-black ${prediction === side ? 'bg-indigo-600 text-white' : 'bg-slate-800 text-slate-400'}`}>{side === 'H' ? 'Heads' : 'Tails'}</button>)}
          </div>
        </div>
        ...
        <div role="status" className="grid grid-cols-2 gap-3 text-center">
      ...
      <div className={`p-5 rounded-3xl border ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'} space-y-4`}>
        ...
        {experiment && <div role="status" className="space-y-2"><div className="flex h-9 overflow-hidden rounded-xl text-xs font-black text-white"><div className="bg-indigo-600 flex items-center justify-center" style={{ width: `${experiment.heads / 10}%` }}>H {experiment.heads}</div><div className="bg-slate-600 flex items-center justify-center" style={{ width: `${experiment.tails / 10}%` }}>T {experiment.tails}</div></div><p className="text-center text-xs text-slate-400">Heads occurred {(experiment.heads / 10).toFixed(1)}%. It varies, but settles near 50% over many trials.</p></div>}
      </div>
    </div>
  );
}
