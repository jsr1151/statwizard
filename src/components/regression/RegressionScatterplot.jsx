...
const RegressionScatterplot = ({
    ...
    const geometry = useMemo(() => {
        ...
        const meanBandPoints = showConfidenceBand && stats?.ok
            ? buildRegressionBand({
                stats,
                confidenceLevel,
            })
            : [];
        const predictionBandPoints = showPredictionBand && stats?.ok
            ? buildPredictionBand({
                stats,
                confidenceLevel,
            })
        ...
        const lineMaxX = stats?.ok
            ? stats.xSummary.max + ((stats.xSummary.max - stats.xSummary.min || 1) * 0.04)
            : paddedMaxX;
        const regressionPoints = stats?.ok
            ...
            : [];
        const confidencePolygon = meanBandPoints.length > 1
            ? [
                ...meanBandPoints.map((point) => ({ x: toSvgX(point.x), y: toSvgY(point.upper) })),
                ...meanBandPoints.slice().reverse().map((point) => ({ x: toSvgX(point.x), y: toSvgY(point.lower) })),
            ]
            : [];
        const predictionPolygon = predictionBandPoints.length > 1
            ? [
        ...
        <div className="space-y-4">
            ...
            <div className={`rounded-2xl border overflow-hidden ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-white border-slate-200'}`}>
                <svg viewBox={`0 0 ${WIDTH} ${HEIGHT}`} className="w-full h-auto">
                    ...
                    {showPredictionBand && geometry.predictionPolygon.length > 2 && (
                        <path
                            d={`${buildPath(geometry.predictionPolygon)} Z`}
                            fill={predictionBandFill}
                            stroke="none"
                        />
                    )}
                    ...
                    {showConfidenceBand && geometry.confidencePolygon.length > 2 && (
                        <path
                            ...
                            fill={confidenceBandFill}
                            stroke="none"
                        />
                    )}

                    {showResiduals && geometry.plottedPairs.map((pair) => (
                ...
                <div className="grid gap-3 md:grid-cols-2">
                    {showLine && (
                        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                            ...
                            <p className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                The line shows the fitted mean of Y at each X value.
                            </p>
                        </div>
                    )}
                    ...
                    {showConfidenceBand && (
                        <div className={`rounded-xl border p-4 ${darkMode ? 'bg-slate-950 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                            ...
                            <p className={`text-sm ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>
                                The band shows uncertainty around the estimated mean line.
                            </p>
                        </div>
                    )}
