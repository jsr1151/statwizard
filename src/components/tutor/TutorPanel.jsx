    ...
    <div className={containerClasses}>
      <div className={`p-4 border-b flex justify-between items-center ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
        ...
        {onClose && (
          <button
            ...
            className={`transition-colors ${darkMode ? 'text-slate-500 hover:text-white' : 'text-slate-400 hover:text-slate-800'}`}
          >
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
        ...
        {level === 'tutor' && (
          <div className={`pt-4 mt-4 border-t space-y-4 animate-in fade-in duration-700 delay-300 ${darkMode ? 'border-slate-800' : 'border-slate-100'}`}>
            ...
            >
              ...
              {showDetail ? <ChevronUp aria-hidden="true" size={10} /> : <ChevronDown aria-hidden="true" size={10} />}
            </button>
        ...
        {script.content?.tryNext && (
          <div
            className={`p-3 rounded-xl border mt-4 space-y-1 animate-in fade-in slide-in-from-bottom-1 duration-500 delay-500 ${darkMode ? 'bg-indigo-600/10 border-indigo-500/20' : 'bg-indigo-50 border-indigo-200'}`}
          >
