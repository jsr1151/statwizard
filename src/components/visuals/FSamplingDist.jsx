...
const FSamplingDist = ({ mode = 'data', fCrit = 4.0, fVal = 0, setFVal, darkMode, df1 = 2, df2 = 27, zoomDist, setZoomDist }) => {
  ...
  const points = useMemo(() => {
    ...
    return pts;
  }, [df1, df2, currentMaxX, distScale, fToX]);
  ...
  const alphaPath = useMemo(() => {
    ...
    return d;
  }, [points, safeFCrit, currentMaxX, fToX, xToF, distScale, df1, df2]);
  ...
  const pValuePath = useMemo(() => {
    ...
    d += `L ${fToX(currentMaxX)},${baselineY} Z`;
    return d;
  }, [points, safeFVal, currentMaxX, fToX, xToF, distScale, df1, df2]);

  return (
    <div className="w-full h-full flex flex-col items-center animate-in fade-in duration-700 overflow-visible">
      <svg
        role="img"
        aria-label={`F distribution with ${df1} and ${df2} degrees of freedom; current F value ${safeFVal.toFixed(2)}`}
      ...
      >
        ...
        <text
          ...
          y={baselineY + 55}
          textAnchor="middle"
          className={`text-[11px] font-black uppercase tracking-[0.5em] ${darkMode ? 'fill-slate-500' : 'fill-slate-400'}`}
        >
          F-Ratio Outcome
        </text>
        ...
        <g transform={`translate(${width - 160}, 60)`} className="select-none">
          ...
          <g transform="translate(10, 15)">
            <ProgressiveTooltip
              as="g"
              term="Alpha Region"
              title="The Rejection Zone"
              desc="The area under the curve that represents the alpha level (e.g., 5%)."
              pedagogy="If your F-ratio falls in this red area, we reject the null hypothesis."
              darkMode={darkMode}
            >
              <rect width="8" height="8" rx="2" fill="#ef4444" opacity="0.6" className="cursor-help" />
            </ProgressiveTooltip>
            <text x="14" y="8" className={`text-[9px] font-black uppercase tracking-widest ${darkMode ? 'fill-slate-400' : 'fill-slate-500'}`}>
              α region
            </text>
          </g>
          <g transform="translate(10, 32)">
            <ProgressiveTooltip
              ...
              term="P-Value"
              title="Observed Tail"
              desc="The probability of getting a result as extreme as yours by random chance."
              pedagogy="Area = p. If this area is smaller than alpha, we have a significant finding."
              darkMode={darkMode}
            >
        ...
        <ProgressiveTooltip
          ...
          desc={`For df1=${df1} and df2=${df2}, F must be > ${safeFCrit.toFixed(2)} to be significant.`}
          darkMode={darkMode}
        >
          <g transform={`translate(${fToX(safeFCrit)}, 45)`} className="cursor-help">
            ...
            <text x="0" y="3" textAnchor="middle" className="text-[11px] font-black fill-rose-500 uppercase tracking-widest">
              Fcrit = {safeFCrit.toFixed(2)}
            </text>
          </g>
        </ProgressiveTooltip>
        ...
        <ProgressiveTooltip
          ...
          term="F-Ratio"
          title="Your Result"
          desc="This is the ratio of treatment variability to error variability calculated from your data."
          darkMode={darkMode}
        >
          <g transform={`translate(${fToX(safeFVal)}, ${Math.max(60, yVal)})`} className="cursor-grab active:cursor-grabbing group">
            ...
            <g transform="translate(0, -50)">
              ...
              <text y="5" textAnchor="middle" className="fill-white text-[14px] font-black">
                F = {safeFVal.toFixed(2)}
              </text>
            </g>
          </g>
        </ProgressiveTooltip>
        ...
        {safeFVal > currentMaxX && (
          <g transform={`translate(${width - 100}, ${baselineY - 100})`} className="animate-pulse">
            ...
            <text y="5" x="-10" textAnchor="middle" className="fill-white text-[10px] font-black uppercase tracking-widest">
              F is off-chart
            </text>
          </g>
        )}
